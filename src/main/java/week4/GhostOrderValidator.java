...
public class GhostOrderValidator {
    ...
    public static class FoodOrder {
        ...
        // No usable no-argument constructor - only parameterized constructor that
        // validates both fields
        public FoodOrder(String studentName, String dishName) {
            if (studentName == null || studentName.trim().isEmpty()) {
                throw new IllegalArgumentException(
                        "Invalid order: Student name cannot be null, blank, or whitespace-only.");
            }
            if (dishName == null || dishName.trim().isEmpty()) {
                throw new IllegalArgumentException(
                        "Invalid order: Dish name cannot be null, blank, or whitespace-only.");
            }
        ...
        public void markDelivered() {
            if (this.isDelivered) {
                System.out.println("Alert: Order for " + studentName + " (" + dishName
                        + ") was already marked delivered! Double-serving detected.");
            } else {
