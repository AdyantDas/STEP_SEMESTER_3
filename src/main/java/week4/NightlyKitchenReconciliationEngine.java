...
public class NightlyKitchenReconciliationEngine {
    ...
    // Alias Premium to PremiumDeliveryAccount for convenience matching example
    // signature
    ...
    public static void processAccount(DeliveryAccount account, double amount, int delayMinutes) {
        ...
        if (account instanceof PremiumDeliveryAccount) {
            ...
            System.out.printf(
                    "Processed Premium Account [%s]: Order Rs %.2f, Settled Rs %.2f, Delay %d min (Base Surge Rs %.2f -> Discounted Surge Rs %.2f)%n",
                    account.getStudentId(), account.getOrderValue(), amount, delayMinutes, surgeFee,
                    discountedSurgeFee);
        } else {
            System.out.printf(
                    "Processed Regular Account [%s]: Order Rs %.2f, Settled Rs %.2f, Delay %d min (Surge Rs %.2f)%n",
                    account.getStudentId(), account.getOrderValue(), amount, delayMinutes, surgeFee);
        }
    }
    ...
    public static void processBatch(DeliveryAccount[] accounts, double[] amounts, int[] delayMinutesArray) {
        ...
        System.out.printf(
                "%n%d processed | %d null skipped | %d premium | %d regular | grand total surge fees = Rs %.2f%n",
