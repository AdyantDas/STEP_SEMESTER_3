...
public class NightlyFleetReconciliationEngine {
    ...
    public static void processAccount(BusTicketAccount account, double amount, int minutesLate) {
        ...
        if (account instanceof SleeperAccount) {
            ...
            System.out.printf(
                    "Processed Sleeper Account [%s]: Base Fare Rs %.2f, Amount Paid Rs %.2f, Late %d min (Penalty Rs %.2f, Sleeper Surcharge Rs %.2f)%n",
                    account.getBookingId(), account.getTicketFare(), amount, minutesLate, penalty, surcharge);
        } else {
            System.out.printf(
                    "Processed Regular Account [%s]: Base Fare Rs %.2f, Amount Paid Rs %.2f, Late %d min (Penalty Rs %.2f)%n",
    ...
    public static void processBatch(BusTicketAccount[] accounts, double[] amounts, int[] minutesLateArray) {
        // Defensive validation: length mismatch corrupts the reconciliation by pairing
        // amounts to the wrong passengers
        ...
        System.out.printf(
                "%n%d processed | %d null skipped | %d sleeper | %d regular | grand total penalties = Rs %.2f%n",
