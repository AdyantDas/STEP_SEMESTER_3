...
public class BusTicketBookingValidator {
    ...
    public static class BusTicket {
        ...
        public BusTicket(String passengerName, String destination) {
            if (!isValidName(passengerName)) {
                throw new IllegalArgumentException("Invalid passenger name: '" + passengerName
                        + "'. Must be non-empty alphabetic characters only.");
            }
            if (!isValidDestination(destination)) {
                throw new IllegalArgumentException(
                        "Invalid destination: '" + destination + "'. Must be non-empty alphabetic characters only.");
            }
        ...
        public void markCheckedIn() {
            if (this.isCheckedIn) {
                System.out.println("Warning: Ticket for " + passengerName + " to " + destination
                        + " is already checked in. Duplicate check-in rejected.");
                return;
            }
            this.isCheckedIn = true;
            System.out.println(
                    "Passenger " + passengerName + " successfully checked in for destination " + destination + ".");
        }
    ...
    public static void processBatch(String[][] rawBookings) {
        ...
        for (String[] booking : rawBookings) {
            ...
            try {
                ...
                String uniqueKey = ticket.getPassengerName().toLowerCase() + "|"
                        + ticket.getDestination().toLowerCase();
        ...
        System.out.println("Valid: " + validCount + " | Rejected: " + rejectedCount + " | Duplicates skipped: "
                + duplicatesSkipped);
    }
